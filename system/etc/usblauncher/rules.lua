...
--[[
-- usb_server_path gets set via commandline argument (-n)
...
-- Uncommment the following section for Carplay
--[[
...
--NOTE: Role swap timeout (in ms) for unsuccessful role swap (or
--      undetected cable removal immediately following device insertion)
role_swap_timeout = 2000

-- uncomment line below to enable Carplay on root port
--root_port_mode_0=blua_root_port_default
...
device(0x1234, 4) { -- these should match the USB descriptors presented to the USB host
	class(USB_CLASS_COMMS, USB_COMMS_SUBCLASS_MODEM, 0) {
		driver"devc-serusb_dcd -e -v -F -s -d iface_list=0,path=$(usb_server_path)"
	},
	class(USB_CLASS_MASS_STORAGE)
	{
		driver"devu-umass_client-block -s $(usb_server_path) -l lun=0,iface=0,fname=/dev/ram0"
	}
}
...
device(0x1234, 0xfff1) { -- these should match the USB descriptors presented to the USB host, the iPod
	class(USB_CLASS_VENDOR_SPECIFIC, 0xF0, 0x0) {
		driver"mm-ipod -d iap2,config=/etc/mm/iap2.cfg,probe,ppsdir=/pps/services/multimedia/iap2 -a i2c,addr=0x11,path=/dev/i2c99 -t usbdevice,path=$(usb_server_path) -l /dev/shmem/iap2.log -vvvvvv";
	}
}
--]]
...
foreach (char_devices) {
	driver"devc-serusb -d vid=$(vendor_id),did=$(product_id),busno=$(busno),devno=$(devno),path=$(usb_server_path),ign_remove";
}
...
device(0x2996, 0x0123) { -- these should match the USB descriptors presented to the USB host
    class(USB_CLASS_HID) {
        driver"devu-ut-jack -o -vvvvv -c vid=$(vendor_id),did=$(product_id),busno=$(busno),devno=$(devno),iface=$(iface) -s $(usb_server_path)"
    };
}

-- basic options for devb-umass
DISK_OPTS  = "mem name=below1G blk memory=below1G,cache=1m,vnode=384,auto=none,delwri=2:2,rmvto=none,noatime disk name=umass cdrom name=umasscd";
UMASS_OPTS = "umass path=$(usb_server_path),priority=21";
--
...
SD_UMASS_OPTS = "umass path=$(usb_server_path),priority=21,mpoll=2000";
...
-- CarPlay devices (iPhone)
product(0x05AC, 0x12A8) {
--    RoleSwap_DigitaliPodOut;
--    RoleSwap_AppleDevice;
--    Probe_iAP2;
    Probe_AppleCapability;

    class(USB_CLASS_AUDIO, USB_AUDIO_SUBCLASS_CONTROL) {
--        inc_usr_spec_id = unique"/fs/ipod";  -- does rsrcdbmgr_devno_attach
        driver"io-audio -dusb busno=$(busno),devno=$(devno),cap_name=ipod-$(busno)-$(devno),usbpath=$(usb_server_path)";
    };
    class(USB_CLASS_HID) {
        custom = function(obj)
            don = device_obj_name(obj);
            rmps_publish(don, "iap::2");
            return "mm-ipod -diap2,config=/etc/mm/iap2.cfg,ppsdir=/pps/services/multimedia/iap2-$(busno)-$(devno) -ai2c,addr=0x11,path=/dev/i2c99 -tusbhost,busno=$(busno),devno=$(devno),path=$(usb_server_path),audio=/dev/snd/ipod-$(busno)-$(devno)";
        end;
        driver"$(custom)";
    };
};
...
--        inc_usr_spec_id = unique"/fs/ipod";  -- does rsrcdbmgr_devno_attach
    ...
    class(USB_CLASS_HID) {
        custom = function(obj)
            don = device_obj_name(obj);
            rmps_publish(don, "iap::1");
            return "io-fs-media -dipod,pps=/pps/services/multimedia/iap1-$(busno)-$(devno),transport=usb:busno=$(busno):devno=$(devno):path=$(usb_server_path):audio=/dev/snd/ipod-$(busno)-$(devno),darates=+8000:11025:12000:16000:22050:24000,playback,acp=i2c:addr=0x11:path=/dev/i2c99,config=/etc/mm/ipod.cfg";
        end;
        driver"$(custom)";
    };
};

-- iap2 devices (non-iPhone)
product(0x05AC, 0x1290, 0x12AA) {
    class(USB_CLASS_AUDIO, USB_AUDIO_SUBCLASS_CONTROL) {
--        inc_usr_spec_id = unique"/fs/ipod";  -- does rsrcdbmgr_devno_attach
        driver"io-audio -dusb busno=$(busno),devno=$(devno),cap_name=ipod-$(busno)-$(devno),usbpath=$(usb_server_path)";
    };
...
device(0x054c, 0x03fd) { -- Sony Walkman
	...
	ms_desc(string.char(0x4d, 0x54, 0x50), "", 1) {
		driver"io-fs-media -dpfs-sony,device=$(busno):$(devno):$(iface):path=$(usb_server_path)";
	};
};
--]]
...
device(0x781, 0x7420) { -- SanDisk sansa
	ms_desc("MTP", "", 0xfe) {
		driver"io-fs-media -dpfs-sansa,device=$(busno):$(devno):$(iface):path=$(usb_server_path)";
	};
};
--]]
...
device() {
	ms_desc("MTP") {
		driver"io-fs-media -dpfs,usbpath=$(usb_server_path),device=$(busno):$(devno):$(iface)";
	};
};
...
-- startncm.sh/stopncm.sh replacement
--[[
device() {
    class(USB_CLASS_COMMS, USB_COMMS_SUBCLASS_NCM) {
        custom_start = function(obj)
            name = "ncm";
            iface = name .. "0";
            mount_cmd = "mount -Tio-pkt -o verbose=0,name=" .. name .. ",path=$(usb_server_path),protocol=ncm,usbdnet_mac=000022446688,mac=020022446688 /lib/dll/devnp-usbdnet.so";
            waitfor_cmd = "if_up -p " .. iface;
            start_cmd = "ifconfig " .. iface .. " up";
            iface_ready = os.execute(mount_cmd .. ";" .. waitfor_cmd);
            if iface_ready == true then
                obj.netiface = iface;
                obj.netiface_drv = "/pps/qnx/driver/1234567890";
                rmps_publish(obj.netiface_drv, "NCM::start");
                return"$(start_cmd)";
            else
                return"echo Fail to mount network driver name=" .. name;
            end;
        end;
        custom_removal = function(obj)
            rmps_publish(obj.netiface_drv, "NCM::stop");
            return"ifconfig " .. obj.netiface .. " destroy";
        end;
        start"$(custom_start)";
        removal"$(custom_removal)";
    };
};
--]]
...
AOA = {
	...
	blacklist = {
		...
		{ vid = 0x2996;              }; -- UT H2H
		{ vid = 0x2c48;              }; -- Delphi H2H
		{ vid = 0x0424;              }; -- Microchip H2H
		{ vid = 0x042f;              }; -- Molex H2H
	};
}
...
------------------------------------------------------------------------------
--[[
-- Turn on to dump lua configuration
if verbose >= 3 then
	table.dump(new_conf)
